...
inline UIImage *YCSDScaledImageForKey(NSString *key, UIImage *image) {
    if (!image) {
        return nil;
    }
    ...
    if ([image.images count] > 0) {
        ...
        for (UIImage *tempImage in image.images) {
            [scaledImages addObject:YCSDScaledImageForKey(key, tempImage)];
        }
...
NSString *const YCSDWebImageErrorDomain = @"SDWebImageErrorDomain";
